...
const defaultSuggestions = [
  ...
  'Cho minh biet chu de noi bat cua KP23',
  'Tim bai viet lien quan den moi truong',
];
    ...
    <div className='mt-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3 text-xs text-slate-600'>
      ...
      <div className='mt-2 space-y-2'>
        {citations.map((citation, index) => {
          ...
          return (
            <div key={`${citation.source}-${index}`} className='rounded-xl bg-white p-3 shadow-sm'>
  ...
  async function handleSend(prefilledMessage) {
    ...
    try {
      ...
      setMessages((current) =>
        current.filter(
          (message) =>
            message.id !== tempUserMessage.id && message.id !== tempAssistantMessage.id
        )
      );
      setDraft(content);
    } finally {
      ...
      <aside className='overflow-hidden rounded-[32px] border border-emerald-100 bg-white/85 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur lg:w-[320px]'>
        <div className='border-b border-emerald-100 bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.2),_transparent_55%),linear-gradient(135deg,_rgba(236,253,245,0.9),_rgba(255,255,255,1))] p-5'>
          ...
          <p className='mt-2 text-sm leading-6 text-slate-600'>
            Tra loi dua tren knowledge base, co nho ngu canh va streaming response.
          </p>
          <button
            type='button'
        ...
        <div className='max-h-[520px] space-y-2 overflow-y-auto p-3'>
          {isLoadingList ? (
            <p className='px-3 py-6 text-sm text-slate-500'>Dang tai hoi thoai...</p>
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              <p className='text-sm font-semibold text-slate-900'>
                Tro ly AI cho user cua KP23&apos;s Blog
              </p>
            ...
            <div className='grid gap-4 lg:grid-cols-[1.2fr_0.8fr]'>
              <div className='rounded-[28px] border border-white/70 bg-white px-5 py-6 shadow-sm'>
                ...
                <h2 className='mt-3 text-3xl font-semibold leading-tight text-slate-900'>
                  Hoi dap tu knowledge base, co bo nho va co streaming
                </h2>
                <p className='mt-4 text-sm leading-7 text-slate-600'>
                  Chatbot nay chi duoc phep tong hop tu tai lieu blog hoac du lieu trong
                  Chroma. Neu knowledge base chua co thong tin, he thong se tra loi
                  rang khong du can cu thay vi tu suy doan.
                </p>
              </div>
              ...
              <div className='rounded-[28px] border border-emerald-100 bg-[linear-gradient(160deg,_rgba(16,185,129,0.08),_rgba(255,255,255,1))] p-5'>
                <p className='text-sm font-semibold text-slate-900'>Goi y cau hoi</p>
        ...
        <div className='border-t border-white/70 bg-white/80 px-4 py-4 backdrop-blur sm:px-6'>
          ...
          >
            <textarea
              ...
              placeholder='Nhap cau hoi ve noi dung blog, tai lieu hoac knowledge base...'
              rows={3}
            ...
            <div className='mt-3 flex items-center justify-between gap-3'>
              <p className='text-xs text-slate-400'>
                Khong bia thong tin. Neu khong co du lieu, chatbot se noi ro.
              </p>
              <button
                type='submit'
